...
{
	public:
		Solver_MCSVM_CS(const problem *prob, int nr_class, double *C, double eps=0.1, int max_iter=1000);
		~Solver_MCSVM_CS();
		void Solve(double *w);
	private:
	...
	double Cp, double Cn, int solver_type)
{
	...
	for(i=0; i<l; i++)
	{
		...
		feature_node *xi = prob->x[i];
		tempi=0;
		while (xi[tempi].index != -1)
		{
			QD[i] += (xi[tempi].value)*(xi[tempi].value);
			tempi++;
		}
#endif
		index[i] = i;
	...
	while (iter < max_iter)
	{
		...
		for (s=0;s<active_size;s++)
		{
			...
			while(xi[tempi].index!= -1)
			{
				G += w[xi[tempi].index-1]*(xi[tempi].value);
				tempi++;
			}
#endif
			G = G*yi-1;
			...
			if(fabs(PG) > 1.0e-12)
			{
				...
				while (xi[tempi].index != -1)
				{
					w[xi[tempi].index-1] += d*xi[tempi].value;
					tempi++;
				}
#endif	
			}
